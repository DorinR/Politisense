...
const VoteParticipantsXmlParser = require('./VoteParticipantsXmlParser')
  .VoteParticipantsXmlParser
...
class VoteXmlParser extends XmlDataParser {
  static getVoteParticipantsUrl (voteId, currentParliament) {
    return (
      'https://www.ourcommons.ca/Parliamentarians/en/HouseVotes/ExportDetailsVotes?' +
  ...
  buildJson () {
    const vote = VoteRecord.builder(
      Number(this.getDataInTag('DecisionDivisionNumber'))
    )
  ...
  isFinalDecision () {
    const voteSubject = this.getDataInTag(
  ...
  async getVoters (voteId) {
    ...
    const voteParticipants = await this._getHtmlFromLink(
      VoteXmlParser.getVoteParticipantsUrl(voteId, this.currentParliament)
    )
    if (voteParticipants === '') {
      return ''
    }
